...
class LianLianKanAutomator:
    ...
    def compare_images(self, img1: np.ndarray, img2: np.ndarray) -> float:
        ...
        main1 = self.preprocess_image(img1, crop_ratio=0.82)
        main2 = self.preprocess_image(img2, crop_ratio=0.82)

        # 统一尺寸，减少局部噪声影响
        main1_resized = cv2.resize(main1, (96, 96), interpolation=cv2.INTER_AREA)
        main2_resized = cv2.resize(main2, (96, 96), interpolation=cv2.INTER_AREA)

        # 方法1：结构相似度 (SSIM)
        ssim_similarity = self.ssim_similarity(main1_resized, main2_resized)

        # 方法2：感知哈希 (pHash)
        phash_similarity = self.phash_similarity(main1_resized, main2_resized)

        # 方法3：边缘相似度
        edge_similarity = self.edge_similarity(main1_resized, main2_resized)

        # 方法4：颜色直方图（HSV）比较
        hist_similarity = self.color_hist_similarity(main1_resized, main2_resized)

        # 底部文字条，增强区分度
        bottom1 = self.extract_bottom_strip(img1)
        bottom2 = self.extract_bottom_strip(img2)
        bottom_similarity = self.text_strip_similarity(bottom1, bottom2)

        # 加权融合
        similarity = (
            0.38 * ssim_similarity
            + 0.22 * phash_similarity
            + 0.18 * edge_similarity
            + 0.07 * hist_similarity
            + 0.15 * bottom_similarity
        )

        return float(max(0.0, min(1.0, similarity)))

    def preprocess_image(self, img: np.ndarray, crop_ratio: float = 0.8) -> np.ndarray:
        """中心裁剪，减少边框与背景干扰"""
    ...
    def extract_bottom_strip(self, img: np.ndarray, height_ratio: float = 0.25) -> np.ndarray:
        """提取底部文字条区域"""
        height, width = img.shape[:2]
        strip_h = max(1, int(height * height_ratio))
        return img[height - strip_h:height, 0:width]

    def text_strip_similarity(self, img1: np.ndarray, img2: np.ndarray) -> float:
        """底部文字条相似度"""
        if img1 is None or img2 is None:
            return 0.0

        resized1 = cv2.resize(img1, (96, 32), interpolation=cv2.INTER_AREA)
        resized2 = cv2.resize(img2, (96, 32), interpolation=cv2.INTER_AREA)

        ssim_similarity = self.ssim_similarity(resized1, resized2)
        edge_similarity = self.edge_similarity(resized1, resized2)
        hist_similarity = self.color_hist_similarity(resized1, resized2)
        return float(max(0.0, min(1.0, 0.5 * ssim_similarity + 0.3 * edge_similarity + 0.2 * hist_similarity)))
